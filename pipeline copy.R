...
count_nonzero <- read.csv(file.path(datasetName, "VectAbundance_2024_nonzero.csv"))
data_clim <- read.csv(file.path(datasetName, "VectAbundance_2024_allclimnonzero.csv"))

# Create dataframes for the analyses ----
## Output directory ----
outdir_df <- file.path(datasetName, "publication")
dir.create(outdir_df, recursive = TRUE, showWarnings = FALSE)
...
## ANALYSIS 1 2010-2015 vs 2016-2022 ----
analysis1_periods <- list(
  "2010_2015" = c(2010, 2015),
  "2016_2022" = c(2016, 2022))

analysis1 <- list()

for(period in names(analysis1_periods)){
  
  yrs <- analysis1_periods[[period]]
  ...
  analysis1[[period]] <- list(
    aedes = aedes_tmp,
    clim  = clim_tmp
  )
  
  dir.create(
    file.path(outdir_df, "analysis_1", period),
    recursive = TRUE,
    showWarnings = FALSE
  )
  ...
  write_csv(
    ...
    file.path(
      ...
      "analysis_1",
      period,
      paste0("clim_", period, ".csv")))
  
}

rm(aedes_tmp, clim_tmp, yrs, period)


## ANALYSIS 2 2010-2014 vs 2015-2018 vs 2019-2022 ----
analysis2_periods <- list(
  "2010_2014" = c(2010, 2014),
  "2015_2018" = c(2015, 2018),
  "2019_2022" = c(2019, 2022)
)

analysis2 <- list()

for(period in names(analysis2_periods)){
  
  yrs <- analysis2_periods[[period]]
  ...
  analysis2[[period]] <- list(
    aedes = aedes_tmp,
    clim  = clim_tmp
  )
  
  dir.create(
    file.path(outdir_df, "analysis_2", period),
    recursive = TRUE,
    showWarnings = FALSE
  )
...
## ANALYSIS 3 Individual years (2010–2022)
analysis3 <- list()
years_all <- sort(unique(data_aedes$year))
...
for(yr in years_all){
  ...
  analysis3[[as.character(yr)]] <- list(
    aedes = aedes_tmp,
    clim  = clim_tmp
  )
  
  dir.create(
    file.path(outdir_df, "analysis_3", as.character(yr)),
    recursive = TRUE,
    showWarnings = FALSE
  )
...
rm(aedes_tmp, clim_tmp, yr, years_all)


## FUNCTION: Aggregate climate data ----
aggregate_ccm_data <- function(data_aedes,
                               data_clim,
                               eco,
                               interval = 7,
                               max_lag = 8){
  ...
  for(id in unique(aedes_eco$ID)){
    
    message("Processing ", eco, " | ID ", id)
    
    clim_sub <- clim_eco |> filter(ID == id)
    aedes_sub <- aedes_eco |> filter(ID == id)
    
    if(nrow(clim_sub) == 0 | nrow(aedes_sub) == 0)
      next
  ...
  clim_agg <- bind_rows(clim_agg_list) |>
    mutate(ecoreg = eco) |>
...
## Run function ----
### ANALYSIS 1 ----
analysis1_merged <- list()

for(period in names(analysis1)){
  
  message("Running Analysis 1: ", period)
  
  analysis1_merged[[period]] <- list()
  
  for(eco in c("Alpine","Continental","Mediterranean")){
    
    merged_data <- aggregate_ccm_data(
      data_aedes = analysis1[[period]]$aedes,
      data_clim  = analysis1[[period]]$clim,
      eco        = eco,
      interval   = 7,
      max_lag    = 8
    )
    
    analysis1_merged[[period]][[eco]] <- merged_data
    ...
    write_csv(
      ...
      file.path(
        ...
        "analysis_1",
        period,
        paste0(eco,"_aggclim.csv")))
  }
  
}

## ANALYSIS 2 ----
analysis2_merged <- list()
for(period in names(analysis2)){
  
  message("Running Analysis 2: ", period)
  
  analysis2_merged[[period]] <- list()
  
  for(eco in c("Alpine","Continental","Mediterranean")){
    
    merged_data <- aggregate_ccm_data(
      data_aedes = analysis2[[period]]$aedes,
      data_clim  = analysis2[[period]]$clim,
      eco        = eco,
      interval   = 7,
      max_lag    = 8
    )
    
    analysis2_merged[[period]][[eco]] <- merged_data
...
## ANALYSIS 3 ----
analysis3_merged <- list()

for(yr in names(analysis3)){
  
  message("Running Analysis 3: ", yr)
  
  analysis3_merged[[yr]] <- list()
  
  for(eco in c("Alpine","Continental","Mediterranean")){
    
    merged_data <- aggregate_ccm_data(
      data_aedes = analysis3[[yr]]$aedes,
      data_clim  = analysis3[[yr]]$clim,
      eco        = eco,
      interval   = 1,
      max_lag    = 56)      # 56 days = 8 weeks

    analysis3_merged[[yr]][[eco]] <- merged_data
...
outdir_ccm <- file.path(outdir_df, "CCM_outputs")
...
for(period in names(analysis1_merged)){
  
  ## Create folder for this period
  out_period <- file.path(out_csv, "analysis_1", period)
  dir.create(out_period, recursive = TRUE, showWarnings = FALSE)
  
  fits_analysis1[[period]] <- list()
  
  for(eco in names(analysis1_merged[[period]])){
    
    message("Running ", period, " | ", eco)
    
    fits_analysis1[[period]][[eco]] <- list()
    
    for(pred in predictors){
      
      message("   Predictor: ", pred)
      
      fit_tbl <- fit_models_by_lag(
        data       = analysis1_merged[[period]][[eco]],
        response   = response,
        predictors = pred,
        random     = random,
        family     = family
      ...
      fits_analysis1[[period]][[eco]][[pred]] <- fit_tbl
      
      write_csv(
        fit_tbl,
        file.path(
          out_period,
          paste0(eco, "_fitmodels_", pred, ".csv")))
    }
  }
}
...
for(period in names(analysis2_merged)){
  
  ## Create folder for this period
  out_period <- file.path(out_csv, "analysis_2", period)
  dir.create(out_period, recursive = TRUE, showWarnings = FALSE)
  
  fits_analysis2[[period]] <- list()
  
  for(eco in names(analysis2_merged[[period]])){
    
    message("Running ", period, " | ", eco)
    
    fits_analysis2[[period]][[eco]] <- list()
    ...
    for(pred in predictors){
      ...
      fit_tbl <- fit_models_by_lag(
        data       = analysis2_merged[[period]][[eco]],
        response   = response,
        predictors = pred,
        random     = random,
        family     = family
      ...
      fits_analysis2[[period]][[eco]][[pred]] <- fit_tbl
...
# fits_analysis3 <- list()
# 
# for(yr in names(analysis3_merged)){
#   
#   ## Create folder for this year
#   out_year <- file.path(out_csv, "analysis_3", yr)
#   dir.create(out_year, recursive = TRUE, showWarnings = FALSE)
#   
#   fits_analysis3[[yr]] <- list()
#   
#   for(eco in names(analysis3_merged[[yr]])){
#     
#     message("Running ", yr, " | ", eco)
#     
#     fits_analysis3[[yr]][[eco]] <- list()
#     
#     for(pred in predictors){
#       
#       message("   Predictor: ", pred)
...
#       )
#       
#       fits_analysis3[[yr]][[eco]][[pred]] <- fit_tbl
#       
#       write_csv(
#         fit_tbl,
#         file.path(
...
## ANALYSIS 1 - CREATE CCM FIGURES

for(period in names(fits_analysis1)){
  
  message("Creating CCM layout: ", period)
  
  ccm_data_list <- list()
  ccm_best_list <- list()
  
  for(eco in names(fits_analysis1[[period]])){
    
    fits_region <- fits_analysis1[[period]][[eco]]
    
    for(pred in names(fits_region)){
      
      message(period, " | ", eco, " | ", pred)
      ...
      ## always keep the full CCM
      ccm_data_list[[paste(eco,pred)]] <- df
      
      ## only create a highlighted best lag if at least one significant lag exists
      if(any(!is.na(df$value))){
        
        idx <- which.max(abs(df$value))
  ...
  ccm_data$predictor <- factor(
    ccm_data$predictor,
    levels = predictors
  )
  
  ccm_best$predictor <- factor(
    ccm_best$predictor,
    levels = predictors
  )
  
  ccm_data$ecoreg <- factor(
    ccm_data$ecoreg,
    levels=c("Alpine","Continental","Mediterranean")
  )
  
  ccm_best$ecoreg <- factor(
    ccm_best$ecoreg,
    levels=c("Alpine","Continental","Mediterranean")
  )
  
  r2_limits <- c(
    min(ccm_data$value,na.rm=TRUE),
    max(ccm_data$value,na.rm=TRUE)
  )
  
  p_all <- ggplot(ccm_data,
                  aes(lag_start,lag_end,fill=value))+
    ...
    labs(
      title=paste0("Analysis 1: ",period),
      x="Lag start (weeks)",
      y="Lag end (weeks)"
    )+
...
## ANALYSIS 2 - CREATE CCM FIGURES
for(period in names(fits_analysis2)){
  ...
  for(eco in names(fits_analysis2[[period]])){
    
    fits_region <- fits_analysis2[[period]][[eco]]
    ...
    for(pred in names(fits_region)){
      ...
      ## only highlight a best lag if at least one significant lag exists
      if(any(!is.na(df$value))){
  ...
  ## Global colour limits for this period
  r2_limits <- c(
    min(ccm_data$value, na.rm = TRUE),
    max(ccm_data$value, na.rm = TRUE)
  )
  ...
  p_all <- ggplot(ccm_data,
    ...
    labs(
      title = paste0("Analysis 2: ", period),
      x = "Lag start (weeks)",
      y = "Lag end (weeks)"
    ) +
