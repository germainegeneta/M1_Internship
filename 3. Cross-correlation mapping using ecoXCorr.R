...
for (eco in ecoregs) {
  ...
  for (v in clim_vars) {
    ...
    fit_tbl <- fit_models_by_lag(data       = merged_eco,
                                 ...
                                 random     = random,
                                 family     = family)
    write_csv(fit_tbl, file.path(out_csv, paste0(eco, "_fitmodels_", v, ".csv")))
...
for (eco in ecoregs) {
  for (v in clim_vars) {
    ...
    ggsave(filename = file.path(out_png, paste0(eco, "_", v, "_full.png")),
...
p_facet <- ggplot(fits_long, aes(lag_start, lag_end, fill = R2_plot)) +
  ...
  labs(title = "Climate-abundance CCMs across ecoregions (full timeseries)",
...
p_facet
ggsave(file.path(outdir_ccm, "Allecoreg_full.png"),
  ...
  for (eco in ecoregs) {
    ...
    for (v in clim_vars) {
      ...
      fit_tbl <- fit_models_by_lag(data       = merged_eco,
                                   ...
                                   random     = random,
                                   family     = family)
      write_csv(fit_tbl, file.path(out_csv, paste0(eco, "_fitmodels_", v, "_", split_id, ".csv")))
  ...
  for (eco in ecoregs) {
    for (v in clim_vars) {
      ...
      p_ind <- make_ccm_plot(data        = fit_tbl,
                             ...
                             show_axes   = TRUE)
      ggsave(filename = file.path(out_png, paste0(eco, "_", v, "_", split_id, ".png")),
             plot = p_ind, width = 8, height = 6, dpi = 300)
    }
  }
}
...
for (eco in ecoregs) {
  ...
  p_facet_eco <- ggplot(fits_long_eco, aes(lag_start, lag_end, fill = R2_plot)) +
    ...
    labs(title = paste0("Climate-abundance CCMs across timeseries splits in ", eco, " ecoregion"),
         x = "Lag start (weeks)", 
         y = "Lag end (weeks)") +
    theme_bw() +
    theme(legend.position = "right",
  ...
  ggsave(filename = file.path(out_png, paste0(eco, "_allsplits.png")),
         plot = p_facet_eco, width = 18, height = 10, dpi = 300)
}

## 7: PLOT THE BEST TIME LAG START AND END ##
# helper: extract best lag per split/ecoreg/clim_var
...
p_lag_div <- ggplot(best_lags_div,
  ...
  labs(title = "Best CCM lag start (top) and end (bottom) across timeseries splits",
...
ggsave(file.path(outdir_ccm, "bestlag_diverging.png"), p_lag_div,
...
p_laglen <- ggplot(best_laglen_plot,
  ...
  scale_y_continuous(breaks = 1:max_lag,
                     limits = c(1, max_lag)) +
  facet_wrap(~ clim_var, nrow = 1) +
  labs(title = "Evolution of best lag length across timeseries splits",
...
p_laglen
ggsave(file.path(outdir_ccm, "bestlag_length.png"), p_laglen,
       width = 16, height = 5, dpi = 300)